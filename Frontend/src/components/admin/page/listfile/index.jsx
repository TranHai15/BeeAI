...
const FileList = () => {
  ...
  // console.log("🚀 ~ FileList ~ filesAdd:", filesAdd);
  ...
  const fetchFiles = async () => {
    try {
      ...
      // console.log("🚀 ~ fetchFiles ~ response.data:", response.data);
      const dataNew = response.data.filter((e) => e.statusFile !== 0);
      // const dataOld = response.data.filter((e) => e.statusFile == 0);
  ...
  const handleDeleteFile = async (fileId) => {
    if (confirm("Ban có muốn ẩn file không??????????")) {
      try {
        await axiosClient.get(`/file/hidden/${fileId}`); // API xóa file
        setFiles(files.filter((file) => file.id !== fileId)); // Xóa file khỏi state
        // alert("File deleted successfully");
        fetchFiles();
        showNotification("Ẩn file thành công");
      } catch (error) {
  ...
  const hendleHien = async () => {
    const newdata = [...filesgoc];
    const dataNew = newdata.filter((e) => e.statusFile == 0);
    setFiles(dataNew);
    setShowDetailsFile(true);
  };
  const hendleAn = async () => {
    const dataNew = filesgoc.filter((e) => e.statusFile !== 0);
    setFiles(dataNew);
    setShowDetailsFile(false);
  };
  const hendleKhoiPhuc = async (fileId) => {
    if (confirm("Ban có muốn Khôi Phục file không??????????")) {
      try {
        await axiosClient.get(`/file/reset/${fileId}`); // API xóa file
        fetchFiles();
        showNotification("Khôi Phục file thành công");
        setShowDetailsFile(true);
      } catch (error) {
        console.error("Error deleting file", error);
        alert("Failed to delete file");
              ...
              <tr>
                ...
                <th className="border px-4 py-2 flex gap-2">
                  ...
                  {!showDetailsFile ? (
                    ...
                    <button onClick={() => hendleAn()}>
                      <img
                        className="w-5  object-contain"
                        src="../../../../src/assets/hien.svg"
                      />
                    </button>
                  )}
                </th>
                ...
                <tr key={file.id}>
                  ...
                  <td className="border px-4 py-2 flex space-x-2">
                    {file.statusFile !== 0 ? (
                      ...
                      <button
                        className=" text-white py-1 px-3 rounded hover:bg-yellow-500"
                        onClick={() => hendleKhoiPhuc(file.id)}
                      >
                        <img
                          className="w-5"
                          src="../../../../src/assets/khoiphuc.svg"
                        />
                      </button>
                    )}
                  </td>
